...
PDF_PATH = DOCS_DIR / "Resume_Service_Detailed_Documentation.pdf"
MD_PATH = DOCS_DIR / "Resume_Service_Detailed_Documentation.md"
...
def generate_imports_diagram() -> Path:
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")

    add_box(ax, 0.38, 0.72, 0.24, 0.14, "main.py\nFastAPI resume service", "#dbeafe")
    add_box(ax, 0.05, 0.42, 0.18, 0.14, "dotenv / os\nload secrets and config", "#dcfce7")
    add_box(ax, 0.27, 0.42, 0.18, 0.14, "FastAPI / Starlette\nrouting, uploads, JSON", "#e0f2fe")
    add_box(ax, 0.49, 0.42, 0.18, 0.14, "PyPDF2 / pdf2image /\npytesseract\ntext extraction", "#fef3c7")
    add_box(ax, 0.71, 0.42, 0.18, 0.14, "SentenceTransformer /\nPinecone / Groq\nAI and vector search", "#fde68a")
    add_box(ax, 0.38, 0.14, 0.24, 0.14, "json / re / typing /\nBytesIO\nutility layer", "#fce7f3")
    ...
    add_arrow(ax, (0.50, 0.72), (0.50, 0.28))

    ax.set_title("Resume Service Dependency Map", fontsize=18, weight="bold", pad=20)
    out = ASSETS_DIR / "resume_service_dependency_map.png"
...
def generate_pipeline_diagram() -> Path:
    ...
    add_box(ax, 0.03, 0.58, 0.13, 0.14, "PDF upload", "#dbeafe")
    add_box(ax, 0.20, 0.58, 0.14, 0.14, "read bytes\nBytesIO", "#dcfce7")
    add_box(ax, 0.38, 0.58, 0.14, 0.14, "PyPDF2\nextract text", "#e0f2fe")
    add_box(ax, 0.56, 0.58, 0.14, 0.14, "OCR fallback\npdf2image +\npytesseract", "#fef3c7")
    add_box(ax, 0.74, 0.58, 0.20, 0.14, "Groq parsing + skill merge +\nresume_query generation", "#fde68a")

    add_box(ax, 0.12, 0.18, 0.18, 0.14, "save extracted text\nand structured JSON", "#fce7f3")
    add_box(ax, 0.40, 0.18, 0.18, 0.14, "SentenceTransformer\n384-dim vector", "#bbf7d0")
    add_box(ax, 0.68, 0.18, 0.18, 0.14, "Pinecone upsert\nor query", "#ede9fe")

    add_arrow(ax, (0.16, 0.65), (0.20, 0.65))
    add_arrow(ax, (0.34, 0.65), (0.38, 0.65))
    add_arrow(ax, (0.52, 0.65), (0.56, 0.65))
    add_arrow(ax, (0.70, 0.65), (0.74, 0.65))
    add_arrow(ax, (0.84, 0.58), (0.21, 0.32))
    add_arrow(ax, (0.84, 0.58), (0.49, 0.32))
    add_arrow(ax, (0.84, 0.58), (0.77, 0.32))

    ax.set_title("Resume Service Processing Pipeline", fontsize=18, weight="bold", pad=20)
    out = ASSETS_DIR / "resume_service_pipeline.png"
    fig.savefig(out, dpi=200, bbox_inches="tight")
    plt.close(fig)
    return out


def generate_endpoint_diagram() -> Path:
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")

    add_box(ax, 0.10, 0.70, 0.18, 0.14, "GET /\nhealth check", "#dbeafe")
    add_box(ax, 0.40, 0.70, 0.22, 0.14, "POST /generate-query\nindex one job into Pinecone", "#dcfce7")
    add_box(ax, 0.72, 0.70, 0.18, 0.14, "GET /search\nsemantic job lookup", "#e0f2fe")
    add_box(ax, 0.22, 0.32, 0.22, 0.14, "POST /analyze-resume\nparse one resume", "#fef3c7")
    add_box(ax, 0.56, 0.32, 0.24, 0.14, "POST /search-jobs-from-resume\nparse resume and query Pinecone", "#fde68a")
    add_box(ax, 0.39, 0.08, 0.22, 0.12, "Shared helpers\ntext extraction, skill merge,\nquery building", "#fce7f3")

    add_arrow(ax, (0.51, 0.70), (0.50, 0.20))
    add_arrow(ax, (0.81, 0.70), (0.56, 0.46))
    add_arrow(ax, (0.33, 0.32), (0.45, 0.20))
    add_arrow(ax, (0.68, 0.32), (0.56, 0.20))

    ax.set_title("Resume Service Endpoint Overview", fontsize=18, weight="bold", pad=20)
    out = ASSETS_DIR / "resume_service_endpoints.png"
    fig.savefig(out, dpi=200, bbox_inches="tight")
    plt.close(fig)
    return out
...
def render_text_section(pdf: PdfPages, section: Section) -> None:
    ...
    for start in range(0, len(lines), page_size):
        ...
        ax.text(0.06, 0.92, "\n".join(page), fontsize=10.2, va="top", linespacing=1.45)
        pdf.savefig(fig)
        plt.close(fig)
...
def render_image_page(pdf: PdfPages, title: str, image_path: Path, caption: str) -> None:
    ...
    img_ax.axis("off")
    ax.text(0.06, 0.14, caption, fontsize=10.2, va="top", wrap=True)
    pdf.savefig(fig)
    plt.close(fig)
...
def build_pdf(sections: Sequence[Section], diagrams: Sequence[Path]) -> None:
    with PdfPages(PDF_PATH) as pdf:
        ...
        ax.axis("off")
        ax.text(0.06, 0.93, "Resume Service Detailed Documentation", fontsize=24, weight="bold", va="top")
        ax.text(0.06, 0.86, "Deep dive into `resume/main.py`, its libraries, workflows, and design choices", fontsize=11.5)
        ax.text(
            0.06,
            0.76,
            "\n".join(
                [
                    "- follows the Python file from top to bottom",
                    "- explains each imported library and why it is used",
                    "- documents every helper and endpoint",
        ...
        ax.text(0.06, 0.61, "Source scope", fontsize=14, weight="bold")
        ax.text(0.06, 0.57, "Primary code file: `resume/main.py`\nSupporting context: local venv package set and generated outputs in `resume/extracted_texts`", fontsize=10.8, va="top")
        pdf.savefig(title)
        plt.close(title)
        ...
        render_image_page(
            ...
            "Dependency Map",
            diagrams[0],
            "This map groups the imported libraries by responsibility: configuration, web routing, PDF/OCR extraction, semantic AI, and general utility logic.",
        )
        render_image_page(
            pdf,
            "Processing Pipeline",
            diagrams[1],
            "This is the main runtime flow for a resume: upload, extract text, OCR fallback, LLM structuring, semantic query generation, and optional vector storage or retrieval.",
        )
        render_image_page(
            pdf,
            "Endpoint Map",
            diagrams[2],
            "The service exposes one health endpoint and four functional endpoints. Most of the logic is shared through helper functions in the same file.",
        )
...
def main() -> None:
    ensure_dirs()
    diagrams = [
        generate_imports_diagram(),
        generate_pipeline_diagram(),
        generate_endpoint_diagram(),
    ]
